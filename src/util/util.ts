...
export class UtilPage {
  ...
  static validatePasswordFormatUpperCase(control: FormControl): any {
    ...
    static validatePasswordFormatLetter(control: FormControl): any {
        var reg = /^((?=\S*?[a-zA-Z]).{0,})\S$/;
        if (!reg.test(control.value)) {
            return {"validatePasswordLetter": true};
        }
  ...
  static validateDocumentValidatorFormat(control: FormControl): any {
    var reg = /^([kK0-9]{1})$/;
    //var reg = /^([0-9]{8}-[0-9Kk])$/;
    if (!reg.test(control.value)) {
      return {"validateDocumentId": true};
  ...
  public static sendSMS(window, data,CityWalletNumber) {
  ...
  public static parseMetadataToJson(objects) {
      //console.log("objeto es"+JSON.stringify(objects))
    let aux: any[];
    aux = [];
    let cont=0;
    ...
    public static parseCurrentBalanceValueToJson(object) {
            try {
                var _metadata = {}
                let metadata = object.currentBalanceValue;
                _metadata = JSON.parse(metadata);
                object.currentBalanceValue = {};
                object.currentBalanceValue = _metadata;
...
//        var regVisa = /^(?:4[0-9]{12}(?:[0-9]{3})?)$/;
  ...
  public static hasInternetConnection(network: Network) {
    ...
    return (true) //true while check plugin
